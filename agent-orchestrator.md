...
model: inherit
description: Guides the repo pipeline — user gives a feature plan, rough plan, or just an idea → convert to PRD plus a persisted structured implementation plan (MODE 1) and send for user review; after approval → automatic implement → validate (review) loop; when MODE 2 PASS and the user supplied a visual reference at idea capture → Visual QA against that reference; optional extra Visual QA when applicable.
is_background: false
---
...
| **3** | **`@.cursor/agents/prd-requirements-engineer.md`** MODE 2 | **`docs/requirements/[feature-name].validation.md`** — authoritative acceptance vs PRD; feature must **fully comply** with the PRD before the next step |
...
- **Failure loop:** If MODE 2 **FAIL** → **`@.cursor/agents/feature-implementer.md`** using the FAIL list from `*.validation.md` → MODE 2 **again**. Repeat **automatically** until **PASS**. Do **not** queue **any** Visual QA (reference or optional) until validation **PASS** and the implementation is accepted as **fully PRD-compliant**.
...
- **After MODE 1 (plan ready):** run the **Plan approval checkpoint**; emit nothing that starts implementation until the user approves.
- **After plan approval:** emit or drive **feature-implementer → MODE 2 → failure loop → reference Visual QA (when PRD lists a step-1 link/image) → optional extra Visual QA** as **one continuous automatic run**, interrupting **only** for true blockers.
...
- Read **`docs/requirements/[feature-name].handoff.json`** when present: **`next_agent`** / **`status`** drive who runs next; never skip validation after implementation unless the user explicitly opts out of checks (**opt-out must occur at or before approval**, not silently mid-flight).

## Completion

- **Minimum done (no user visual reference):** MODE 2 **PASS** — point to **`docs/requirements/[feature-name].validation.md`** (and PRD).
